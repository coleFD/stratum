...
impl Downstream {
    ...
    pub async fn init_difficulty_management(self_: Arc<Mutex<Self>>, init_target: &[u8]) -> ProxyResult<'static, ()> {
        let (connection_id, upstream_difficulty_config, miner_hashrate) = self_
            .safe_lock(|d| {
                ...
                (d.connection_id, d.upstream_difficulty_config.clone(), d.difficulty_mgmt.min_individual_miner_hashrate)
            })
        ...
        Self::send_message_upstream(self_, DownstreamMessages::SetDownstreamTarget(
            SetDownstreamTarget { channel_id: connection_id, new_target: init_target.into() }
        )).await;
...
mod test {
    ...
    async fn test_diff_management() {
        ...
        Downstream::init_difficulty_management(downstream.clone(), initial_target.inner_as_ref()).await.unwrap();

...
use roles_logic_sv2::{
    parsers::Mining,
    utils::Mutex,
    Error as RolesLogicError, mining_sv2::UpdateChannel,
};
use tracing::debug;
...
impl Upstream {
    ...
    pub(super) async fn try_update_hashrate(self_: Arc<Mutex<Self>>) -> ProxyResult<'static, ()> {
        let (channel_id_option, diff_mgmt, tx_frame) = self_.safe_lock(|u| 
            (
                u.channel_id.clone(),
        ...
        let should_update_channel_option = diff_mgmt.safe_lock(|c| {
